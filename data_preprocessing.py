...
def step_download(cfg: Dict[str, Any], out_csv: str) -> None:
    ...
    with open(out_csv, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=fieldnames, quoting=csv.QUOTE_MINIMAL, escapechar="\\"
        )
        writer.writeheader()
        for row in tqdm(dataset, total=total_rows, desc="Writing rows"):
            writer.writerow(
                {
                    key: str(row.get(key, "")).replace("\r", " ").replace("\n", " ")
...
def step_zero_shot(cfg: Dict[str, Any], in_csv: str, out_csv: str) -> None:
    ...
    device_index = -1 if device == 'cpu' else int(zs_cfg.get("device", 3))
...
def run_pipeline(config_path: str, steps: List[str] = None) -> None:
    ...
    if steps is None or len(steps) == 0:
        steps = cfg.get("general", {}).get("steps")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Subset of steps to run; if omitted, uses config general.steps or defaults to all",
    )
    parser.add_argument(
    ...
    if args.force:
        ...
        paths = build_paths(cfg)
        steps_to_run = args.steps or cfg.get("general", {}).get("steps")
        logging.info("Force mode enabled. Re-running steps: %s", steps_to_run)
        if steps_to_run["download"]:
            step_download(cfg, paths["download_csv"])
        if steps_to_run["keyword_filter"]:
            step_keyword_filter(cfg, paths["download_csv"], paths["filtered_csv"])
        if steps_to_run["zero_shot"]:
            step_zero_shot(cfg, paths["filtered_csv"], paths["zero_shot_csv"])
